...
/*
    ...
    Philippe Rivest (@technophil98)
    Modificatioon Mars 2018
*/
...
int baudRateMode = USBASP_MODE_SETBAUD2400;
long baudRate = 2400;
...
void afficherAide (void ) {
    fprintf (stderr, "\n" );
    fprintf (stderr, "usage: serieViaUSB [-l | -e] [-nb <n>] [-f <fichier>] [-h | -d | -b] [-s <n>] [-B <baud rate>]\n" );
    fprintf (stderr, "\n" );
    ...
    fprintf (stderr, "\n" );
    fprintf (stderr, "-B --baud-rate <baud rate>: Permet l'ajustement de la vitesse\n" );
    fprintf (stderr, "                  de transmission sérielle. (Défaut 2400)\n" );
    fprintf (stderr, "\n" );
    fflush(0);
    exit (-1);
}
...
/*!
 * By Philippe Rivest (@technophil98)
 * Returns USBASP Baud rate mode for specified Baud rate
 * @param baudRate The baud rate to set
 * @return The Baud rate mode
 */
int baudRateToMode(long baudRate) {

    for (int i = 0; i < BAUD_RATES_AVAILABLE; ++i)
        if (baudRate == BAUD_RATES[i])
            return USBASP_MODE_SETBAUD_SMALLEST + i;

    return -1; //Baud rate un-supported
}
    ...
    while ( i < argc ) {
        ...
        if ( strcmp (argv[i], "-nb") == 0 ||
            ...
            if ( i < argc ) {
                ...
                baudRateMode = baudRateToMode(baudRate);

                if (baudRateMode == -1){
                    fprintf (stderr, "Erreur: Baud rate invalide pour -b ou --baud-rate\n\n");
                    afficherAide();
                }
            }
            else {
                fprintf (stderr, "Erreur: argument manquant pour -b ou --baud-rate\n\n");
                afficherAide();
            }
        }
